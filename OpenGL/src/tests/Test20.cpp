...
namespace test
{
		...
		m_ClearColor(glm::vec3(0.0f))
	{
		...
		m_DebugQuad = std::make_unique<Shape>(quadVertices, sizeof(quadVertices), debugLayout, planeIndices, 6);

		m_WoodTexture = std::make_unique<Texture>("res/textures/wood.png");

		m_DepthMap = std::make_unique<FrameBuffer>(Texture::Type::DEPTH_MAP, 1024, 1024);

		m_Shader = std::make_unique<Shader>("res/shaders/Test20/vertex.glsl", "res/shaders/Test20/fragment.glsl");
		m_DepthShader = std::make_unique<Shader>("res/shaders/Test20/simple_depth_vertex.glsl", "res/shaders/Test20/simple_depth_fragment.glsl");
		m_DebugDepthShader = std::make_unique<Shader>("res/shaders/Test20/debug_depth_vertex.glsl", "res/shaders/Test20/debug_depth_fragment.glsl");

		m_Camera = std::make_unique<Camera>(glm::vec3(0.0f, 0.0f, 3.0f));
	}

	Test20::~Test20() = default;

	void Test20::OnUpdate(float time, float deltaTime)
	{
	...
	void Test20::OnRender()
	{
		...
		//renderer.SetCullFace(Renderer::CullFace::FRONT);
		m_DepthMap->Bind();
		renderer.Clear();
		m_DepthShader->Bind();
		...
		//renderer.SetCullFace(Renderer::CullFace::BACK);
		...
		m_DepthMap->BindTextureBuffer(1);
		// Set light uniforms
		...
		//m_DebugDepthShader->SetUniform1i("depthMap", 0);
		//m_DebugDepthShader->SetUniform1f("near_plane", near_plane);
		//m_DebugDepthShader->SetUniform1f("far_plane", far_plane);
		//m_DepthMap->BindTextureBuffer(0);
		//renderer.Draw(*m_DebugQuad, *m_DebugDepthShader);
		
	}

	void Test20::OnImGuiRender()
	{
	...
	void Test20::RenderScene(Shader& shader)
	{
		Renderer renderer;
